...
- **The drafting model is not Claude.** It currently runs on `openai/gpt-oss-120b` via Groq's free tier, because no Anthropic API credit was available. The call is plain HTTP and the Claude prices are already in the database. `cost_usd` is therefore 0.00: the tier is free, not the pipeline cheap.
